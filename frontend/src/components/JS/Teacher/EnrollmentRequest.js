  ...
  useEffect(() => {
    const fetchEnrollmentRequests = async () => {
      try {
        ...
        const response = await axios.get("http://localhost:5050/api/teacher/view-enrollment-requests", {
          headers: {
  ...
  return (
    ...
    <div className={styles.dashboard1}>
      ...
      {loading ? (
        ...
        <div>No pending enrollment requests.</div>
      ) : (
          ...
          <tbody>
            {requests.map((request) => (
              <tr key={request.request_id}>
                ...
                <td>{new Date(request.request_date).toLocaleDateString('en-GB')}</td>
                <td>
                  <ul>
                ...
                <td>
                  <button onClick={() => handleApprove(request.request_id, request.login_id)}>Approve</button>
